...
evidence (e.g., purchase order, delivery note), and optional grouped `paymentInstructions` and
`taxSubtotal` containers.
...
├── evidenceReferences[]               (optional array)
│   └── evidenceReferencesType
└── status                             (optional; lifecycle for corrections/cancellations/credit notes)
```
...
| `status` | N/A (see code list 2.8) | Lifecycle status for corrections/cancellations/credit notes. | tstr | `active` |

### 2.4 Conditional attributes

*The following members belong to the optional containers/arrays introduced in Section 2.3. Their
presence and obligation are conditional on the presence of their parent group: if the parent group is
present, the members marked "M (within group)" SHALL be present, and those marked "O (within group)"
MAY be present.*
...
*NOTE Each `evidenceReferences` entry carries the evidence itself in one of the supported forms defined
in Section 2.8 (inline PDF/UBL as BASE64, or a publicly accessible URI plus hash). The `dueDate`
attribute referenced in integrity rule IR-EI-05 is reserved for a future profile and is not defined in
this version.*
...
| `status` (metadata) | [SD-JWT VC] / [Token Status List] | Reference to the Token Status List entry for revocation (see Chapter 6). Distinct from the business `status` attribute in Section 2.3. | object | *(see Chapter 6)* |
...
| `evidenceReferences` | Inline `PDF as BASE64`; inline `UBL as BASE64`; `URI identifier + hash` | Supported forms for carrying/referencing a supporting evidence item. | WE BUILD SC5 domestic vocabulary | If a URI form is used, the link MUST be publicly accessible (rationale: no standard authorization mechanism exists for protected URIs). |
...
| `status` | `active`, `corrected`, `cancelled`, `credited` | Lifecycle status of the invoice for corrections/cancellations/credit notes. | WE BUILD SC5 domestic vocabulary | State transitions SHALL follow the rulebook-defined transitions (see IR-EI-08). |
...
| `IR-EI-09` | For corrections/cancellations/credit notes, the attestation SHALL reference the original invoice/attestation and use only rulebook-defined `status` state transitions (see code list 2.8). | Maintains an auditable, well-formed correction lifecycle. | Issuer business rules; verifier state validation. | Verifier SHALL reject an invalid or unreferenced state transition. |
...
| `evidenceReferences` | evidenceReferences | array | Members in 2.4; forms in 2.8 | MAY |
| `status` | status | string | Business lifecycle (2.3); distinct from `status` metadata | MAY |
...
their integrity and validity (IR-EI-08). If all checks pass, the Buyer's Wallet accepts the invoice and
...
### 6.2 Relationship between revocation and the business `status` attribute

Cryptographic revocation (via the Token Status List, Section 6.1) is distinct from the business-level
`status` attribute (`active`/`corrected`/`cancelled`/`credited`, Section 2.3). The business `status`
attribute and the correction/credit-note lifecycle (IR-EI-09) express the accounting state of the
invoice, while Token Status List revocation expresses the cryptographic validity of the attestation. An
issuer correcting or cancelling an invoice SHOULD issue a new attestation with the appropriate `status`
value referencing the original, and SHALL revoke the superseded attestation via the Token Status List
where it should no longer be relied upon.
